...
class TextToVideoSDPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def initialize_latents_with_conditional(self, latents, cond_latents, m):
        """
        latents: (B, 4, F, H, W)
    ...
    def style_transfer_to_nearest_cond(self, latents, cond_frame_indices):
        F = latents.shape[2]
        for f in range(F):
            if f in cond_frame_indices:
                continue
            nearest = min(cond_frame_indices, key=lambda x: abs(x - f))
            latents[:, :, f, :, :] = self.adaptive_instance_normalization(
                latents[:, :, f, :, :],
                latents[:, :, nearest, :, :]
            )
        return latents
    ...
    def prepare_latents(
        ...
        if latents is None:
            ...
            latents =self.scheduler.add_noise(
                ...
                timesteps=(torch.ones(latents.shape[0]) * self.scheduler.num_train_timesteps * (
                        1 - 0.5)).long(),
            )
        return latents
    ...
    def __call__(
            ...
            prompt_embeds: Optional[torch.FloatTensor] = None,
            bbx:Optional[torch.FloatTensor] = None,
            negative_prompt_embeds: Optional[torch.FloatTensor] = None,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                bbx_input = torch.cat([bbx] * 2) if do_classifier_free_guidance else bbx
