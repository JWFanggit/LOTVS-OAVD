...
from attentions import Transformer2DModel,TransformerTemporalModel

from attentions import TemporalConvLayer
...
def custom_checkpoint(module, mode=None):
    ...
    if mode == 'temp':
         def custom_forward(hidden_states, num_frames=None,bbx=None):
...
def transformer_g_c(transformer, sample, num_frames,bbx):
    sample = g_c(custom_checkpoint(transformer, mode='temp'), 
        sample, num_frames,bbx, use_reentrant=False
    )['sample']
...
def cross_attn_g_c(
    ...
    def ordered_g_c(idx):
        ...
        if idx == 0: return g_c(custom_checkpoint(attn, mode='attn'),
            hidden_states, encoder_hidden_states,cross_attention_kwargs,bbx,use_reentrant=False
        )['sample']

        # Temporal Self and CrossAttention
        if idx == 1: return g_c(custom_checkpoint(temp_attn, mode='temp'), 
            hidden_states, num_frames,bbx, use_reentrant=False)['sample']
    ...
    bbx=None):
        ...
        for attn, temp_attn, resnet, temp_conv in zip(
            ...
            if self.gradient_checkpointing:
                ...
                if num_frames > 1:
                    hidden_states = temp_attn(hidden_states, num_frames=num_frames,bbx=bbx).sample
