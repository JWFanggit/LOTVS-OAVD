...
def monkeypatch_or_replace_lora_extended(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if isinstance(_child_module, (nn.Linear, LoraInjectedLinear)):
            ...
            _tmp.linear.weight = nn.Parameter(weight)
            _tmp.linear.bias = nn.Parameter(bias) if bias is not None else None

        elif isinstance(_child_module, (nn.Conv2d, LoraInjectedConv2d)):
            ...
            _tmp.conv.weight = nn.Parameter(weight)
            _tmp.conv.bias = nn.Parameter(bias) if bias is not None else None

        elif isinstance(_child_module, (nn.Conv3d, LoraInjectedConv3d)):
